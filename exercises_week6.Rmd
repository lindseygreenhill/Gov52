...
* Constant term: $logit^{-1}(-4.846) = $ The coefficient for educ1 is .174. In
...
The table below shows how the model classified each prediction. The model only
predicted that respondents would not sign the survey and did not predict any
successes. As such, the error rate is calculated by dividing the incorrectly
classified observations (bottom) by the sum of all of the observations. In this
...
tab_mat <- table(mill_df$signed, predict > .5)
tab_mat
```
